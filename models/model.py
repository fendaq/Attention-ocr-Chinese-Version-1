...
class Model(object):
  ...
  def pool_views_fn(self, nets):
    ...
    with tf.variable_scope('pool_views_fn/STCK'):
      ...
      dim_1 = net.get_shape().dims[1].value
      dim_2 = net.get_shape().dims[2].value
      # feature_size 也就是通道的大小
      feature_size = net.get_shape().dims[3].value

      # return tf.reshape(net, [batch_size, -1, feature_size])
      return tf.reshape(net, shape=[tf.shape(net)[0], dim_1 * dim_2, feature_size])

  def char_predictions(self, chars_logit):
    """Returns confidence scores (softmax values) for predicted character.
  ...
  def create_base(self,
    ...
    with tf.variable_scope(scope, reuse=reuse):
      ...
      # print('shape of CNN feature output: {}'.format(tf.shape(nets[0]).as_list()))
      logging.debug('Conv tower: %s', nets[0])

      # encode_coordinates_fn 没有用到
      nets = [self.encode_coordinates_fn(net) for net in nets]
      logging.debug('Conv tower w/ encoded coordinates: %s', nets[0])

      # 将nets中的图水平拼接起来，并进行reshape，变成了[batch_size, -1, feature_size], feature_size
      # 就是nets中的卷积通道数目
      net = self.pool_views_fn(nets)
      logging.debug('Pooled views: %s', net)

      # 根据是否使用“注意力”和是否使用“自动回归”模型创建相应的解码模型
      chars_logit = self.sequence_logit_fn(net, labels_one_hot)
      chars_logit_tmp = tf.identity(chars_logit, name='chars_logit_tmp')
      logging.debug('chars_logit: %s', chars_logit)

      # 计算最后预测的字符和预测字符的置信度【先通过softmax计算出所有字符的log_prob，然后选择log_prob最大的对应的字符作为最后预测的字符，
      # 然后计算对应预测的字符的置信度】
      predicted_chars, chars_log_prob, predicted_scores = (self.char_predictions(chars_logit))
      chars_log_prob_tmp = tf.identity(chars_log_prob, name='chars_log_prob_tmp')
      predicted_chars_tmp = tf.identity(predicted_chars, name='predicted_chars_tmp')
      predicted_scores_tmp = tf.identity(predicted_scores, name='predicted_scores_tmp')
      ...
      predicted_text_tmp = tf.identity(predicted_text, name='predicted_text_tmp')
  ...
  def create_loss_v2(self, ori_labels, endpoints):
    """Creates all losses required to train the model.
    ...
    # as part of GetTotalLoss. We need to use total loss because model may have
    # multiple losses including regularization losses.
    self.sequence_loss_fn(endpoints.chars_logit, ori_labels)
  ...
  def sequence_loss_fn(self, chars_logits, chars_labels):
    ...
    with tf.variable_scope('sequence_loss_fn/SLF'):
      ...
      batch_size = tf.shape(chars_logits)[0]
      seq_length = chars_logits.get_shape()[1]
